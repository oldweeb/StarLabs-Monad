...
class Monadking:
    ...
    async def get_nft_balance(self) -> int:
        ...
        """
        try:
            # Метод 2: Пробуем использовать tokensOfOwner
            try:
                tokens = await self.nft_contract.functions.tokensOfOwner(
                    self.account.address
                ).call()
                logger.info(f"[{self.account_index}] Monad King NFTs owned: {len(tokens)}")
                return len(tokens)
            except Exception as e:
                logger.debug(f"[{self.account_index}] tokensOfOwner not available: {e}")

            # Метод 3: Стандартный balanceOf
