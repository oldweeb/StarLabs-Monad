...
class Accountable:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code != 200:
                    raise Exception(
                        f"Failed to get signature. Status code: {response.status_code}"
                    )
                ...
                if nonce >= self.config.ACCOUNTABLE.NFT_PER_ACCOUNT_LIMIT:
                    logger.warning(
                        f"[{self.account_index}] Account already minted {nonce} NFTs (limit: {self.config.ACCOUNTABLE.NFT_PER_ACCOUNT_LIMIT})"
                    )
                    return None

                logger.success(
                    f"[{self.account_index}] Successfully got mint signature for token #{token_id} (minted: {nonce})"
                )
                ...
                if attempt == max_retries - 1:
                    logger.error(
                        f"[{self.account_index}] Failed to get signature after {max_retries} attempts: {str(e)}"
                    )
                    return None
                logger.error(
                    f"[{self.account_index}] Attempt {attempt + 1} failed: {str(e)}"
                )
                await asyncio.sleep(
                    random.randint(
    ...
    async def get_nft_balances(self) -> list:
        ...
        try:
            ...
            logger.error(
                f"[{self.account_index}] Error checking NFT balances: {str(e)}"
            )
            return []
        ...
        for retry in range(self.config.SETTINGS.ATTEMPTS):
            try:
                ...
                if not available_nfts:
                    logger.success(
                        f"[{self.account_index}] Already own all available NFTs"
                    )
                    return True
                ...
                logger.info(
                    f"[{self.account_index}] Selected token ID {token_id} for minting from available IDs: {available_nfts}"
                )

                signature = await self.get_mint_signature(token_id)
                if not signature:
                    logger.success(
                        f"[{self.account_index}] Skipping mint - NFT limit reached (CHECK LIMIT IN CONFIG)"
                    )
                    return True

                logger.info(
                    f"[{self.account_index}] Minting NFT #{token_id} with signature"
                )
                ...
                logger.info(
                    f"[{self.account_index}] Waiting for mint transaction confirmation..."
                )
                receipt = await self.web3.eth.wait_for_transaction_receipt(tx_hash)

                if receipt["status"] == 1:
                    logger.success(
                        f"[{self.account_index}] Successfully minted NFT #{token_id}. TX: {EXPLORER_URL}{tx_hash.hex()}"
                    )
                    return True
                else:
                    logger.error(
                        f"[{self.account_index}] Mint transaction failed! TX: {EXPLORER_URL}{tx_hash.hex()}"
                    )
                    return False
                ...
                if "insufficient funds" in str(e).lower():
                    logger.error(
                        f"[{self.account_index}] Insufficient funds to cover gas costs"
                    )
                    return False
                ...
                logger.error(
                    f"[{self.account_index}] Error minting NFT: {str(e)}. Sleeping for {random_pause} seconds"
                )
