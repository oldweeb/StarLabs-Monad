...
class NerzoMonadId:
    def __init__(
        ...
        self.web3 = AsyncWeb3(
            AsyncWeb3.AsyncHTTPProvider(
                RPC_URL,
                request_kwargs={"proxy": (f"http://{proxy}") if proxy else None if proxy else None, "ssl": False},
            )
        )
    ...
    async def mint(self):
        for retry in range(self.config.SETTINGS.ATTEMPTS):
            try:
                logger.info(f"[{self.account_index}] Minting Nerzo Monad ID")
                ...
                if receipt["status"] == 1:
                    logger.success(
                        f"[{self.account_index}] Successfully minted Nerzo Monad ID. TX: {EXPLORER_URL}{tx_hash.hex()}"
                    )
                    return True
                else:
                    logger.error(
