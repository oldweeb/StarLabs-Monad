...
class BeanDex:
    ...
    async def generate_swap_data(
        ...
        try:
            ...
            logger.info(
                f"Setting swap deadline to {deadline} ({time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(deadline))})"
            )
    ...
    async def swap(self, percentage_to_swap: float, type: str = "swap") -> str:
        try:
            ...
            if type == "collect":
                ...
                logger.info(
                    f"Found tokens to collect: {[t[0] for t in tokens_to_swap]}"
                )
                ...
                for token_in, balance in tokens_to_swap:
                    try:
                        ...
                        if current_allowance < amount_wei:
                            logger.info(
                                f"Approving {balance} {token_in} for Bean router"
                            )
                            ...
                            logger.info(
                                f"Sleeping {random_pause} seconds after approve"
                            )
                        ...
                        tx_data = await self.generate_swap_data(
                            token_in, "native", amount_wei, 0
                        )
                        ...
                        logger.error(
                            f"Failed to collect {token_in} to native: {str(e)}"
                        )
                        continue
                ...
                if token_in == "native":
                    ...
                    logger.info(
                        f"Available output tokens for native: {available_tokens}"
                    )
                else:
                    # If swapping from token, can swap to native or other tokens except WMON
                    available_tokens = ["native"] + [
                        t for t in BEAN_TOKENS if t not in [token_in, "wmon"]
                    ]
                    logger.info(
                        f"Available output tokens for {token_in}: {available_tokens}"
                    )
                ...
                if token_in == "native":
                    amount_wei = int(
                        self.web3.to_wei(balance * (percentage_to_swap / 100), "ether")
                    )
                    ...
                    amount_token = float(
                        Decimal(str(amount_wei)) / Decimal(str(10**decimals))
                    )
                    logger.info(f"Swapping {amount_token} {token_in} to {token_out}")
                ...
                tx_data = await self.generate_swap_data(
                    token_in, token_out, amount_wei, min_amount_out
                )
