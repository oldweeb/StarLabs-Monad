...
class AmbientDex:
    def __init__(
        ...
        self.router_contract = self.web3.eth.contract(
            address=self.web3.to_checksum_address(AMBIENT_CONTRACT), abi=AMBIENT_ABI
        )
        self.config = config
    ...
    async def generate_swap_data(
        ...
        try:
            ...
            token_address = (
                AMBIENT_TOKENS[token_out.lower()]["address"]
                if is_native
    ...
    async def approve_token(self, token: str, amount: int) -> str:
        ...
        try:
            token_contract = self.web3.eth.contract(
                address=self.web3.to_checksum_address(
                    AMBIENT_TOKENS[token.lower()]["address"]
                ),
                abi=ERC20_ABI,
            )
            ...
            if receipt["status"] == 1:
                logger.success(
                    f"Approval successful! Explorer URL: {EXPLORER_URL}{tx_hash.hex()}"
                )
                return tx_hash.hex()
            else:
    ...
    async def swap(self, percentage_to_swap: float, type: str) -> str:
        ...
        try:
            ...
            if type == "collect":
                ...
                tokens_to_swap = [
                    (t, b) for t, b in tokens_with_balance if t != "native"
                ]
                ...
                for token_in, balance in tokens_to_swap:
                    try:
                        ...
                        logger.info(
                            f"Swapping {balance} {token_in} to MON. Sleeping {random_pause} seconds after approve"
                        )
