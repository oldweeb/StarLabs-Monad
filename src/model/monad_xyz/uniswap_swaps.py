...
class MonadSwap:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    logger.error(
                        f"Error getting balance after {max_retries} attempts: {str(e)}"
                    )
                else:
    ...
    async def calculate_amount(
        self, percentage_to_swap: float, token_out: str
    ) -> float:
        ...
        if balance_ether < MIN_AMOUNT:
            logger.warning(
                f"Balance too small for {balance_token}: {balance_ether} (minimum: {MIN_AMOUNT})"
            )
            return 0

        balance_wei = self.web3.to_wei(balance_ether, "ether")
        balance_wei_percentage = int(balance_wei * percentage)
        balance_ether_percentage = float(
            round(
                self.web3.from_wei(balance_wei_percentage, "ether"),
                random.randint(2, 8),
            )
        )
        ...
        if Decimal(str(balance_ether_percentage)) < MIN_AMOUNT:
            logger.warning(
                f"Calculated swap amount too small: {balance_ether_percentage} (minimum: {MIN_AMOUNT})"
            )
            return 0

        logger.info(f"Balance: {balance_ether} {balance_token}")
        logger.info(
            f"Swapping {percentage_to_swap}% = {balance_ether_percentage} {balance_token}"
        )

        return balance_ether_percentage

    async def _generate_url_percentage(
        self, percentage_to_swap: float, token_out: str
    ) -> Dict[str, Any]:
        """Get a quote for swapping tokens."""
        amount_after_percentage = await self.calculate_amount(
            percentage_to_swap, token_out
        )
    ...
    async def _generate_url_amount(
        self, amount: float, token_in: str
    ) -> Dict[str, Any]:
    ...
    async def get_swap_quote(
        self, percentage_to_swap_or_amount: float, token_out: str, token_in: str = None
    ) -> Dict:
        max_retries = 5
        ...
        if token_out == "native":
            url = await self._generate_url_amount(
                percentage_to_swap_or_amount, token_in
            )
        else:
            url = await self._generate_url_percentage(
                percentage_to_swap_or_amount, token_out
            )
                ...
                try:
                    ...
                    if isinstance(response_data, dict) and "error" in response_data:
                        ...
                        if "number greater than" in error_msg:
                            logger.warning(
                                f"Balance too small for swap, skipping: {response_data['error']}"
                            )
                            return None

                    if not response_data.get("transaction"):
                        raise ValueError(
                            f"No transaction data in response: {response_data}"
                        )
                    ...
                    if attempt == max_retries - 1:
                        raise Exception(
                            f"Failed to get quote after {max_retries} attempts: {str(e)}"
                        )
    ...
    async def generate_approve_transaction(
        self, token: str, amount: float, swap_tx_data: Dict
    ) -> Dict:
        """
        ...
        try:
            ...
            logger.info(
                f"Generated approve transaction for {amount} {token} to spender {spender_address} (Gas: {gas_limit})"
            )
            return tx_data
        ...
        try:
            if token_out == "native":
                ...
                for token, balance in tokens_with_balance:
                    swap_tx_data = await self.get_swap_quote(
                        balance, "native", token_in=token
                    )
                    approve_tx_data = await self.generate_approve_transaction(
                        token, balance, swap_tx_data
                    )
                    ...
                    logger.info(
                        f"Swapping {balance} {token} to MON. Sleeping {random_pause} seconds after approve"
                    )
