...
class IzumiDex:
    ...
    async def generate_swap_data(
        ...
        try:
            ...
            token_in_address = (
                IZUMI_TOKENS["wmon"]["address"]
                if token_in == "native"
                else IZUMI_TOKENS[token_in]["address"]
            )
            token_out_address = (
                IZUMI_TOKENS["wmon"]["address"]
                if token_out == "native"
                else IZUMI_TOKENS[token_out]["address"]
            )
            ...
            recipient = (
                IZUMI_CONTRACT if token_out == "native" else self.account.address
            )

            # Encode swapAmount call
            swap_data = self.router_contract.encode_abi(
                "swapAmount", [(path, recipient, amount_in, min_acquired, deadline)]
            )
            ...
            multicall_data = self.router_contract.encode_abi(
                "multicall", [multicall_array]  # Pass as a single array argument
            )
