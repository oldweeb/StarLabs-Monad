...
from src.model.help import BestCaptchaSolver
...
async def faucet(
    ...
    for retry in range(config.SETTINGS.ATTEMPTS):
        try:
            return True
            # solver = BestCaptchaSolver(
            #     config.CAPTCHA.PROXY_FOR_CAPTCHA,
            #     config.CAPTCHA.BESTCAPTCHA_API_KEY,
            # )
            # for _ in range(config.CAPTCHA.CAPTCHA_SOLVE_ATTEMPTS):
            #     result = solver.solve_recaptcha(
            #         "6LerLrcqAAAAAFSznddAF6KufR5mW4jsrfMYvjg1",
