...
class Superboard:
    ...
    async def quests(self):
        try:
            ...
            campaigns = [
                "nads-on-testnet-faucets",
                "nads-on-testnet-explore-apriori",
                "monad-madness-101-explore-uniswap",
                "nads-on-testnet-explore-izumi-finance",
                "nads-on-testnet-madness",
                "nads-on-testnet-explore-magma",
                "nads-on-testnet-explore-name-service",
                "nads-on-testnet-explore-nostra-finance",
                "nads-on-testnet-explore-shmonad",
                "nads-on-testnet-explore-kintsu",
                "nads-on-testnet-explore-mach-finance",
                "nads-on-testnet-explore-bubblefi",
                "nads-on-testnet-explore-ambient",
            ]
            ...
            for campaign in campaigns:
                ...
                logger.info(
                    f"{self.account_index} | Sleeping {random_pause} seconds after {campaign}"
                )
    ...
    async def _register(self, payload, signature):
        try:
            ...
            response = await self.session.post(
                "https://api-prod.superboard.xyz/api/trpc/user.register",
                params=params,
                headers=headers,
                json=json_data,
            )
    ...
    async def _complete_campaign(self, campaign_data: dict):
        try:
            ...
            if "userQuest" not in quest_info:
                ...
                raise Exception(f"{quest['name']} - Failed to start quest")

            user_quest = quest_info["userQuest"][0]
            tasks_taken = [task["taskId"] for task in user_quest["tasksTaken"]]

            if user_quest["status"] == "REWARD_CLAIMED":
                logger.success(
                    f"{self.account_index} | {quest['name']} - Campaign already completed"
                )
                return True
            ...
            for task in quest_info["tasks"]:
                ...
                while retry_count < max_retries and not task_completed:
                    try:
                        ...
                        retry_count += 1
                        if "Transactions not found" in str(e):
                            logger.warning(
                                f"{self.account_index} | {task['name']} - Task is not completed"
                            )
                            return False
            ...
            if "user must complete all the task" in str(e):
                logger.info(
                    f"{self.account_index} | {quest['name']} - Task is not completed"
                )
                return True
            ...
            if "User already claimed the reward" in str(e):
                ...
                logger.error(
                    f"{self.account_index} | Error completing campaign {quest['name']}: {e}"
                )
                return False

    @retry_async(attempts=3, default_value=None)
